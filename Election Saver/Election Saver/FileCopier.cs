...
namespace Election_Saver
{
    ...
    internal class FileCopier
    {
        ...
        public bool testBitLockerPermission()
        {
            ...
            {
                //bitManager.UnlockDriveWithPassphrase(bitLockerPassword);
                DriveInfo tempDrive = new DriveInfo("C");
                BitLockerManager bitManagerTest = new BitLockerManager(tempDrive);
                bitManagerTest.IsLocked();
                //bitManager = null;
            }
            catch (Exception Error)
            {
        ...
        public void giveCurrentUserBitlockerPermission()
        {
            ...
            MessageBox.Show("The application will now restart to apply the permission changes.", "Application restart message", MessageBoxButton.OK);
